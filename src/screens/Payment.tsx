...
const Payment: React.FC<PaymentScreenProps> = (
	...
	const [isLoadingTour, setLoadingTour] = useState<boolean>(false);
	const [isTourListOpen, setTourListOpen] = useState<boolean>(false);
	const [isNewTourFormOpen, setNewTourFormOpen] = useState<boolean>(false);
	const [isOverlayModalOpen, setOverlayModalOpen] = useState(false);

	const totalAmount: number = props.route.params.orderAmount;
	const currency: string = props.route.params.currency;
	...
	const initializePaymentSheet = async () => {
		...
		//Required for Cart
		if (typeof props.route.params.cartId !== "undefined") {
			reqData.cartId = props.route.params.cartId;
		}

		//Required for Bookings & E-SIM
		if (typeof props.route.params.productId !== "undefined") {
			reqData.productId = props.route.params.productId;
		}

		//Required for Bookings
		if (typeof props.route.params.bookingId !== "undefined") {
			reqData.bookingId = props.route.params.bookingId;
		}
		if (typeof props.route.params.bookingNo !== "undefined") {
			reqData.bookingNumber = props.route.params.bookingNo;
		}
		...
		try {
			const intentResponse: any = await PaymentService.getIntent(reqData);
	...
	const onPaymentSuccess = () => {
		...
		PaymentService.verify(reqData)
			.then((response: any) => {
				setProcessing(false);
				setPaymentFailed(false);
				setPaymentSuccess(true);
				setSearchData([]);
				setUserData({ ...(userData as UserDataModel), total_cart_items: 0 });
			})
			.catch((error) => {
				setProcessing(false);
				setPaymentSuccess(false);
	...
	const onCloseTourList = () => {
		setTourListOpen(false);
	};

	const onOpenTourCreateForm = () => {
		setNewTourFormOpen(true);
	};

	const onCloseJourneyCreateForm = () => {
		setNewTourFormOpen(false);
	};

	const assignOrderToJourney = (
		journeyID: string,
		journeyTitle: string,
		tabIndex: number
	) => {
		setOverlayModalOpen(true);
		const reqData = { journeyId: journeyID, orderId: orderId };
		...
		TourBookService.orderToTour(reqData)
			.then((response: any) => {
				...
				setTimeout(() => {
					props.navigation.navigate("JourneyDetails", {
						journeyID,
						journeyTitle,
						tabIndex,
					});
	...
	const renderItem = ({ item }: any) => {
		return (
			<TouchableHighlight
				...
				onPress={() => assignOrderToJourney(item._id, item.name, 1)}
			>
				<>
					<View style={{ width: "20%" }}>
						<ListAvatar>
	...
	const onSubmitNewTour = (values: any) => {
		Keyboard.dismiss();
		setOverlayModalOpen(true);
		...
		TourBookService.create(reqData)
			.then((response: any) => {
				...
				const itemsArr = [...tours];
				itemsArr.unshift(response.data);
				setTours(itemsArr);
				setNewTourFormOpen(false);
				assignOrderToJourney(resData._id, values.journeyName, 1);
			})
			.catch((error) => {
				setOverlayModalOpen(false);
			});
						...
						<View style={{ flex: 1, width: "100%" }}>
							...
							>
								...
								<Text style={[styles.subText, { fontSize: 14 }]}>
									{"Your payment has been processed & order is placed."}
								</Text>
						...
						>
							{({
								...
								<View style={{ flex: 1, width: "100%", padding: 20 }}>
									...
									>
										<Button
											title={LocalizedText.CANCEL}
											onPress={onCloseJourneyCreateForm}
											titleStyle={{ color: Colors.primaryBtn }}
											style={{
												width: 80,
												height: 35,
