...
export default class DepartureTab extends React.Component<Props, States> {
		...
		getFlightList(reqData)
			.then((response) => {
				if (Array.isArray(response)) {
						...
						(item: any, index: number) => {
							...
							if (index2 < response.length) {
								...
								const secElementTime: moment.Moment = moment(
									response[index2].departure.scheduledTime
								).tz(timeZone);
								...
								const firstElementTime: moment.Moment = moment(
									response[index - 1].departure.scheduledTime
								).tz(timeZone);
								const secElementTime: moment.Moment = moment(
									item.departure.scheduledTime
								).tz(timeZone);
					...
					this.setState({ flightLoading: false, flightData: response }, () => {
						setTimeout(() => {
			...
			<View style={styles.container}>
				<ScrollView showsVerticalScrollIndicator={false} ref={this.scroller}>
					{this.state.flightData.map((item, index) => {
						let delay = getFlightDelay(item.departure.delay);
								...
								<View style={{ flexDirection: "row" }}>
									...
									<View style={{ width: "30%", alignItems: "flex-end" }}>
										<Text
											...
											ellipsizeMode="tail"
										>
											{item.departure.terminal !== null
												? `T${item.departure.terminal}`
												: "-"}
										</Text>
									</View>
								</View>
